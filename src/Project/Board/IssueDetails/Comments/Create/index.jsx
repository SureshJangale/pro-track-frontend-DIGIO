...
const ProjectBoardIssueDetailsCommentsCreate = ({ issueId, fetchIssue }) => {
  ...
  const handleCommentCreate = async () => {
    try {
      setCreating(true);
      await api.post(`/create-comment`, { body, issueId, userId: get(currentUser, "_id") });
      await fetchIssue();
      setFormOpen(false);
      setCreating(false);
      setBody('');
  ...
  return (
    <Create>
      {currentUser && <UserAvatar name={currentUser.name} avatarUrl={getUserAvatarUrl(currentUser)} />}
      <Right>
        {isFormOpen ? (
          <BodyForm
            value={body}
