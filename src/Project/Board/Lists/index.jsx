...
const ProjectBoardLists = ({ project, filters, updateLocalProjectIssues }) => {
  ...
  const handleIssueDrop = ({ draggableId, destination, source }) => {
    ...
    api.optimisticUpdate(`/issues/${draggableId}`, {
      updatedFields: {
        status: destination.droppableId,
        listPosition: calculateIssueListPosition(project.issues, destination, source, draggableId),
      },
      currentFields: project.issues.find(({ _id }) => _id === draggableId),
      setLocalData: fields => updateLocalProjectIssues(draggableId, fields),
    });
  };
...
const getAfterDropPrevNextIssue = (allIssues, destination, source, droppedIssueId) => {
  ...
  const droppedIssue = allIssues.find(issue => get(issue, '_id') === droppedIssueId);
  const isSameList = destination.droppableId === source.droppableId;
