        ...
        <List>
          ...
          >
            {filteredListIssues.map((issue, index) => (
              <Issue key={get(issue, "_id")} projectUsers={project.users} issue={issue} index={index} />
            ))}
...
const filterIssues = (projectIssues, filters, currentUserId) => {
  ...
  if (userIds.length > 0) {
    issues = issues.filter(issue => intersection(issue.assignees, userIds).length > 0);
  }
  if (myOnly && currentUserId) {
    issues = issues.filter(issue => issue.assignees.includes(currentUserId));
  }
  if (recent) {
